...
export function fetchContagemStatusOrdens() {
    ...
    fetch('/usinagem/api/indicador-planejado-concluido-hoje/')
        .then(response => {
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
...
function retornarMaquina(maquina) {
    Swal.fire({
        ...
        text: `Deseja retornar a maquina ${maquina} a producao?`,
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const refreshStatus = document.getElementById('refresh-status-maquinas');
    if (refreshStatus) {
    ...
    const refreshPecas = document.getElementById('refresh-pecas');
    if (refreshPecas) {
        refreshPecas.addEventListener('click', function () {
            fetchUltimasPecasProduzidas();
        });
    }

    const refreshOrdens = document.getElementById('refresh-ordens');
    if (refreshOrdens) {
        refreshOrdens.addEventListener('click', function () {
            fetchContagemStatusOrdens();
        });
    }
