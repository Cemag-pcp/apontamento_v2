...
function badgeSituacao(emAndamento) {
    return emAndamento
...
function renderRows(rows) {
    ...
    tbody.innerHTML = rows.map((row) => `
        <tr>
            ...
            <td>${escapeHtml(row.pecas_faltantes || '-')}</td>
            <td>${escapeHtml(row.data_inicio || '-')}</td>
            <td>${row.em_andamento ? badgeSituacao(true) : escapeHtml(row.data_fim || '-')}</td>
...
async function carregar() {
    ...
    try {
        ...
        tbody.innerHTML = `<tr><td colspan="7" class="text-center text-danger py-4">Falha ao carregar: ${escapeHtml(err.message)}</td></tr>`;
...
// Pré-preenche o inicio da parada com 30 dias atras
function preencherDataPadrao() {
    const hoje = new Date();
    const trintaDiasAtras = new Date(hoje);
    trintaDiasAtras.setDate(hoje.getDate() - 30);
    const fmt = (d) => d.toISOString().slice(0, 10);
    document.getElementById('filtroDataIni').value = fmt(trintaDiasAtras);
