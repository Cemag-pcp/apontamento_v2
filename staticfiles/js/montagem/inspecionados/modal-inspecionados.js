    ...
    document.addEventListener("click", function(event) {
        ...
        if (event.target.closest(".timeline-item")) { 
            ...
            if(parseFloat(naoConformidade) > 0) {
                ...
                const modalHistorico = document.getElementById("modal-historico-montagem");
                const listaCausas = document.getElementById("causas-montagem");
                ...
                const modalCausas = new bootstrap.Modal(document.getElementById("modal-causas-historico-montagem"));
                modalCausas.show();

                fetch(`/inspecao/api/historico-causas-montagem/${id}`, {
                    method:"GET",
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRFToken': document.querySelector('[name=csrfmiddlewaretoken]').value
