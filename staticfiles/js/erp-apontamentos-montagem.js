...
function bindRowActionButtons() {
    tbody.querySelectorAll('.btn-erp-apontar').forEach((btn) => {
        btn.addEventListener('click', () => {
            try {
                apontamentoSelecionado = JSON.parse(btn.getAttribute('data-row'));
    ...
    tbody.querySelectorAll('.btn-erp-detalhes').forEach((btn) => {
        btn.addEventListener('click', () => {
            try {
                openDetailsModal(JSON.parse(btn.getAttribute('data-row')));
            } catch (error) {
                console.error(error);
            }
        });
    ...
    tbody.querySelectorAll('.btn-erp-ver-erro').forEach((btn) => {
        btn.addEventListener('click', () => {
            try {
                openErrorModal(JSON.parse(btn.getAttribute('data-row')));
            } catch (error) {
                console.error(error);
            }
        });
...
function renderRows(rows) {
    ...
    tbody.innerHTML = visibleRows.map((row) => `
        <tr data-item-id="${Number(row.id)}">
            ...
            <td class="text-center">${renderActionButton(row)}</td>
        </tr>
    `).join('');
...
function openConfirmModal(row) {
    ...
    modalResumoEl.textContent = `Ordem ${row.ordem || '-'} | Conjunto ${row.peca_codigo || '-'} | Qtd. boa ${formatNumber(row.qtd_boa)} | Data produzida ${row.data_producao || '-'}`;
...
function openDetailsModal(row) {
    ...
    detalhesResumoEl.textContent = `Ordem ${row.ordem || '-'} | Conjunto ${row.peca_codigo || '-'} | Qtd. boa ${formatNumber(row.qtd_boa)}`;
    detalheApontadoStatusEl.textContent = (row.ordem_ja_apontada || row.apontado) ? 'Apontado' : 'Nao apontado';
...
function openErrorModal(row) {
    ...
    erroResumoEl.textContent = `Ordem ${row.ordem || '-'} | Conjunto ${row.peca_codigo || '-'} | Tipo ${row.tipo_apontamento || '-'}`;
    erroDescricaoEl.textContent = row.erro_apontamento || 'Sem descricao de erro.';
...
function apontarUrl(itemId) {
    return apontarUrlTemplate.replace(/\/0\/apontar\/?$/, `/${itemId}/apontar/`);
}
...
async function confirmarApontamentoSelecionado(tipoApontamento = 'manual') {
    ...
    try {
        const response = await fetch(apontarUrl(selectedId), {
            method: 'POST',
            headers: {
        ...
        if (!response.ok || payload.status !== 'success') {
            ...
            throw new Error(payload.description || payload.retorno_api || payload.message || 'Falha ao confirmar apontamento.');
        }
        ...
        if (btnConfirmarApontamentoERP) {
            ...
            btnConfirmarApontamentoERP.innerHTML = previousText || 'Apontar manualmente';
        }
        if (btnApontarViaApiERP) {
...
function bindEvents() {
    ...
    btnAtualizarTabela?.addEventListener('click', () => loadPage(state.page || 1));
    ...
    btnConfirmarApontamentoERP?.addEventListener('click', () => confirmarApontamentoSelecionado('manual'));
    btnApontarViaApiERP?.addEventListener('click', () => confirmarApontamentoSelecionado('api'));
}
