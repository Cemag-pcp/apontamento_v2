...
def planejamento(request):
    ...
    motivos = MotivoInterrupcao.objects.filter(setor__nome='usinagem')
    operadores = Operador.objects.filter(setor__nome='usinagem')

    return render(request, 'apontamento_usinagem/planejamento.html', {'motivos':motivos,'operadores':operadores})

def get_pecas_ordem(request, pk_ordem, name_maquina):
    try:
        # Busca a ordem com os relacionamentos necessários
        ordem = Ordem.objects.prefetch_related('ordem_pecas_usinagem').get(ordem=pk_ordem, grupo_maquina=name_maquina)
...
def get_ordens_criadas(request):
    ...
    ordens_queryset = Ordem.objects.prefetch_related('ordem_pecas_usinagem').filter(
    ...
    for ordem in ordens_page:
        peca_info = ordem.ordem_pecas_usinagem.first()  # Assume que há apenas uma peça por ordem
