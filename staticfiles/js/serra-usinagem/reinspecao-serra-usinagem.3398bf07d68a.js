...
document.getElementById("btn-filtrar-reinspecao-serra-usinagem").addEventListener("click", (event) => {
    event.preventDefault();
    buscarItensReinspecao(1); // Chama a função quando o botão de filtro é clicado, começando na página 1
});

document.getElementById("btn-limpar-reinspecao-serra-usinagem").addEventListener("click", (event) => {
...
function buscarItensReinspecao(pagina) {
    ...
    fetch(`/inspecao/api/itens-reinspecao-serra-usinagem/?${params.toString()}`, {
        method: 'GET',
        headers: {
